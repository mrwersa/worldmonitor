---
title: "健康检查端点"
description: "World Monitor 的两个健康检查端点用于监控数据管道完整性，在 Vercel Edge Runtime 上运行并查询 Redis (Upstash) 评估状态。"
---
...
**身份验证：** 无需身份验证。对所有来源开放（`Access-Control-Allow-Origin: *`）。
...
| `COVERAGE_PARTIAL` | 警告 | 记录数低于该键声明的 `minRecordCount`（例如 10/13 个咽喉点） |
...
`recordCount` 是实时信号密度覆盖：CII 刷新期间存在的与评分相关的 Tier-1 ACLED、新闻和网络信号系列的数量。
当这些订阅源可达但安静时，`riskScores` 仍可报告
...
# 仅在 Redis 不可达时返回 503（REDIS_DOWN）；其他所有状态均返回 200
# 判定（HEALTHY/WARNING/DEGRADED/UNHEALTHY）位于响应体的 `status` 字段中。
```
...
当种子的年龄超过**配置间隔的 2 倍**时，即被视为过期。这考虑了 cron/中继计时的正常抖动。
...
使用 `/api/health` 作为监控器 URL。HTTP 状态码仅区分 Redis 完全中断与其他所有情况：
...
> 裸 `/api/health` URL 会对响应体进行美化打印，因此其中的健康令牌是 `"status": "HEALTHY"`，冒号后**带空格**。如果您监控非紧凑 URL，请匹配该带空格的令牌 —— 无空格的 `"status":"HEALTHY"` 即使在健康时也不存在，会导致误报告警。优先使用 `?compact=1`，或解析 `summary.crit > 0`。
