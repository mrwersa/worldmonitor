---
title: "航运 v2 API"
description: "Shipping v2 API 提供咽喉要道路由情报查询以及供应链中断警报的 webhook 订阅管理端点 —— 完整请求 / 响应参考。"
---
...
注册用于咽喉要道中断警报的 webhook。返回 `201 Created`。
...
- `alertThreshold` —— 数值 0-100（默认 `50`）。超出此范围的值返回 `400 "alertThreshold must be a number between 0 and 100"`。
