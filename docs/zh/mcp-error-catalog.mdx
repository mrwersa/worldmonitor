---
title: "MCP 错误目录"
description: "WorldMonitor MCP 服务器可能返回的每种错误形态：JSON-RPC 代码、HTTP 状态码与软行为信封，附触发条件、负载和处理建议。"
---
...
- **配额回滚对软信封自动执行**。`_budget_exceeded` 和工具执行错误（`-32603`）会退还 Pro 每日配额槽位 —— 你无需为你无法使用的响应付费。`_jmespath_error` **不**退还，因为工具抓取已成功；失败的是用户提供的投影。
...
在 `api/mcp/auth.ts` 的五个站点触发，总是配对 HTTP **401** 和 `WWW-Authenticate` 头。五个触发点按客户端命中顺序排列如下：
...
**如何处理。** 重新走一遍 OAuth 流程（`/api/oauth/token` 使用新的授权码，或用有效 refresh token 做刷新授权）。对于 API key 客户端，复核 `X-WorldMonitor-Key` 头 —— `wm_live_…` 形式的 key 必须放在该头中，**而不是**作为 `Authorization: Bearer`。`WWW-Authenticate` 头中的 `resource_metadata` 指针是从零开始发起发现流程的权威入口。

### `-32029` —— 被限流（每分钟或每日）

两种不同触发条件共用此代码；由 HTTP 状态码区分。

**每分钟节流 —— HTTP 200。** 滑动窗口限流器为 **60 次请求 / 分钟**，按 API key（Starter 及以上）或按 Pro 用户（该用户所有令牌合计）键控。限流器运行在 `api/mcp/handler.ts` 中，位于方法分发 switch **之前**，因此到达它的**每一个**请求都计数 —— `tools/call`、所有元数据方法（`tools/list`、`describe_tool`、`prompts/list`、`resources/list`），以及生命周期方法（`initialize`、`notifications/initialized`、`ping`）。没有按方法的豁免。（Pro 每日上限的豁免集合是独立的且更窄 —— 见下文。）以 HTTP 200 内的 JSON-RPC 错误返回，因为限流器位于任何按 id 关联的上游。在 Upstash 瞬时错误时**故障开放** —— 限流器后端的偶发延迟尖峰不会拖垮整个 API。

`message` 文本标识了是哪个限流器触发。有两种不同字符串：
...
**Pro 每日上限 —— HTTP 429 + `Retry-After`。** 硬性每日上限（Pro 默认 **50 次工具调用 / UTC 日**；API 层级有更大额度）由工具运行**之前**的原子 Redis 预留执行，因此恰好跨越边界的那次调用会被拒绝。只有 `tools/call` 和 `resources/read`（鉴权对称的 resources 路径）计数。**豁免每日上限的方法：** `describe_tool`、`tools/list`、`prompts/list`、`prompts/get`、`resources/list`、`logging/setLevel`、`initialize`、`notifications/initialized`、`ping`。（这些方法仍计入上面的每分钟限制 —— 每日与每分钟的豁免集合不同。）
...
**如何处理。** 对于 HTTP 200 / 每分钟：退避约 1 秒后重试。限流器是滑动窗口而非令牌桶 —— 持续 60 rpm 没问题；任意 60 秒窗口内超过 60 的突发会被拒绝。对于 HTTP 429 / 每日：遵循 `Retry-After`（该值为 `距 UTC 午夜的秒数`）；若每日上限是瓶颈约束，升级到更高层级（API Starter 1,000/天、API Business 10,000/天、Enterprise 不限）。
...
`method` 字段是字符串但未匹配到任何 handler。本服务器支持的方法：`initialize`、`notifications/initialized`、`ping`、`tools/list`、`tools/call`、`prompts/list`、`prompts/get`、`resources/list`、`resources/read`、`logging/setLevel`。
...
| 触发条件                                                | 站点                              | 示例 `message`                                                          |
|--------------------------------------------------------|-----------------------------------|----------------------------------------------------------------------------|
| `tools/call` 缺少或 `name` 非字符串                 | `api/mcp/dispatch.ts:113`         | `Invalid params: missing tool name`                                        |
| `tools/call` 的 `name` 不在注册表中    | `api/mcp/dispatch.ts:117`         | `Unknown tool: get_foo`                                                    |
| `prompts/get` 缺少或 `name` 非字符串                | `api/mcp/handler.ts:133`          | `Invalid params: missing prompt name`                                      |
| `prompts/get` 名称未知或缺少必填参数 | `api/mcp/prompts/index.ts:302`    | `Unknown prompt: …` / `Missing required argument "country_code" for prompt "country-briefing"` |
| `resources/read` 缺少/未知/格式错误的 `uri`       | `api/mcp/resources/index.ts:200`  | `Invalid params: missing resource uri` / `Unknown resource uri "..."` |
| `logging/setLevel` 级别非字符串或不在集合内 | `api/mcp/handler.ts:156`          | `Invalid params: level must be one of debug, info, notice, warning, error, critical, alert, emergency` |
...
**如何处理。** 阅读 `message` —— 它总是告诉你缺少或错误了什么。对于工具，名称在 `tools/list` 中。对于提示，名称 + 参数 schema 在 `prompts/list` 中。对于资源，四种支持的 URI 形状在 `resources/list` 中。对于 `logging/setLevel`，有效级别是上面列出的 [RFC 5424 子集](https://www.rfc-editor.org/rfc/rfc5424#section-6.2.1)。

### `-32603` —— 内部错误

三种不同条件共用此代码；HTTP 状态码区分重试是否合理。

**HTTP 200 —— 工具执行失败。** 工具分发器抛出异常。最常见情形：工具读取的每个 Redis key 都返回 null（`cache_all_null` —— 瞬时 Redis 抖动或仍在预热的种子程序），或一个同侪内部抓取在调用中途失败。Pro 配额会自动回滚；你无需为此付费。
...
| 触发条件                                         | `message`                                              | 站点                              |
|-------------------------------------------------|--------------------------------------------------------|-----------------------------------|
| OAuth 解析服务抛出（Convex 抖动）    | `Auth service temporarily unavailable. Try again.`     | `api/mcp/auth.ts:142`             |
| `MCP_INTERNAL_HMAC_SECRET` 未设置（Pro 路径）     | `Service temporarily unavailable, retry in a moment.`  | `api/mcp/auth.ts:204`             |
| Pro 配额预留 Redis 失败（非上限）   | `Service temporarily unavailable, retry in a moment.`  | `api/mcp/dispatch.ts:141`         |
...
| **403** | 纯文本 `"Forbidden"`       | n/a（**非** JSON-RPC）| `Origin` 头存在且不是 `https://claude.ai` 或 `https://claude.com`。服务器间调用者（无 `Origin`）豁免。 |
| **405** | 空                     | n/a              | 请求方法不是 `POST`、`HEAD` 或 `OPTIONS`。设置了 `Allow: POST, HEAD, OPTIONS` 头。 |
...
有两个 HTTP 状态码不属于 JSON-RPC 错误：

- **403 携带纯文本 `Forbidden` 主体**来自 JSON-RPC 层运行**之前**的 origin 校验。不要尝试对主体做 `JSON.parse`。这只与浏览器来源的客户端相关；CLI 客户端、Claude Desktop 和 `curl` 不发送 `Origin` 头，因此豁免。
- **405 携带空主体**来自 JSON-RPC **之前**的方法校验。handler 接受 `POST`（JSON-RPC 路径）、`HEAD`（以 200 ack 携带 `Content-Type: application/json`，供可用性探测使用）和 `OPTIONS`（CORS 预检）。其他方法得到 405 + `Allow: POST, HEAD, OPTIONS`。
...
**配额。** Pro 每日配额槽位会自动回滚。你无需为你无法使用的响应付费。
...
- [MCP Server 概览](/zh/mcp-server) —— 端点、鉴权模式、OAuth 配置、套餐与配额。
