---
title: "设计哲学"
description: "World Monitor 背后的设计原则、情报工艺与算法决策 —— 以地图为先的仪表盘如何工程化以及为何这样构建。"
---
...
| **浏览器优先计算**           | 分析（聚类、不稳定性评分、激增检测）在客户端运行 — 核心情报不依赖后端计算。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **多源佐证**      | 关键情报信号使用多个独立来源以减少单一来源偏差。抗议数据合并 ACLED + GDELT 并进行 Haversine 去重。国家风险融合新闻速率 + 军事活动 + 动荡事件 + 基线风险。灾害数据在 0.1° 地理网格上合并 USGS + GDACS + NASA EONET。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **类型安全数据流**             | 可辨识联合标记（`_kind` 字段）、proto 生成的带类型客户端/服务端，以及穷尽式 `switch` 匹配，确保 15+ 种标记类型、34 个服务领域和 56 个地图图层的编译时安全。添加新数据类型会在每个未处理位置产生编译错误。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
**竞争假设分析（ACH）** — 多源佐证要求（新闻 + 军事 + 市场 + 抗议活动相互收敛后才升级为关键）是 ACH 的自动化形式。任何单一数据流都无法单独触发关键告警 — 系统要求跨独立数据流的收敛，从而降低单一来源报告错误或宣传运动的影响。

**情报缺口意识** — 专业情报评估总是注明它们*不知道*的内容。数据新鲜度追踪器显式报告"无法看到的内容" — 31 个来源及其状态分类（新鲜、过期、非常过期、无数据、错误、已禁用）。两个来源（GDELT、RSS）被标记为 `requiredForRisk`，意味着它们的缺失会直接影响 CII 评分质量。当关键数据源宕机时，系统显著展示该缺口，而不是静默省略，防止因数据不全而产生虚假信心。
...
**击杀链意识** — 突发新闻告警管道的 5 来源设计对应情报击杀链概念。RSS 告警提供初始检测；关键词激增确认新兴叙事；热点升级和军事激增提供佐证信号；OREF 警报提供地面真相。每个来源都增加了评估的可信度。
...
**边缘网关** — 单个 Vercel Edge Function（`api/[domain]/v1/[rpc].ts`）将所有 22 个 `createServiceRoutes()` 函数导入为一个扁平的 `Map<string, handler>` 路由器。每个 RPC 是一个静态路径上的 POST 端点（例如，`POST /api/aviation/v1/list-airport-delays`），带 CORS 强制执行、隐藏 5xx 响应内部细节的顶层错误边界，以及限流支持（429 时 `retryAfter`）。同一个路由器通过 Vite dev-server 插件（`vite.config.ts` 中的 `sebufApiPlugin`）在本地运行，并在处理器变更时进行 HMR 失效。
...
21 个 Railway 定时任务持续用从外部 API 预计算的数据刷新 Redis 缓存。种子按可配置计划运行（通常每 5–15 分钟），同时写入规范域名键（用于 RPC 处理器查找）和引导键（用于页面加载水合）。这种双键策略确保引导水合和 RPC 处理器在数据格式和新鲜度上始终一致。
...
| `seed-cyber-threats` | Feodo、URLhaus、C2Intel、OTX、AbuseIPDB | 10 分钟 | `cyber:threats-bootstrap:v2` |
...
| `seed-natural-events` | USGS + GDACS + NASA EONET | 10 分钟 | `natural:events:v1` |
...
| `seed-ucdp-events` | UCDP GED API | 15 分钟 | `conflict:ucdp-events:v1` |
...
| `seed-security-advisories` | 通过中继代理的 24 个政府 RSS 源 | 1 小时 | `intelligence:advisories-bootstrap:v1` |
...
- **RSS 代理** — 域名白名单代理，支持 500+ 订阅源，防止 CORS 问题并隐藏源服务器。来自屏蔽 Vercel IP 的域名的订阅源自动通过 Railway 中继路由。
...
- **金融地理数据** — 证券交易所（92 个）、金融中心（19 个）、央行（13 个）和大宗商品枢纽（10 个）作为带类型的静态数据集提供，含市值、GFCI 排名、交易时间和大宗商品专长
...
- **公司丰富化** — `IntelligenceService.GetCompanyEnrichment` 将 GitHub 组织数据、推断的技术栈（由按星数加权的仓库语言分布得出）、SEC EDGAR 公开文件（10-K、10-Q、8-K）和 Hacker News 提及聚合到单一响应中。`IntelligenceService.ListCompanySignals` 呈现实时公司活动信号 — 融资事件、招聘激增、高管变动和扩张公告 — 来源于 Hacker News 和 GitHub，每个按信号类型分类并基于参与度、评论量和时效性评分
...
所有五个仪表盘变体（World Monitor、Tech Monitor、Finance Monitor、Commodity Monitor、Happy Monitor）从**单一 Vercel 部署**提供服务。变体在运行时通过主机名检测确定：
...
`def()` 工厂函数减少了每条目的样板代码。变体特定的图层排序（`VARIANT_LAYER_ORDER`）为五个仪表盘变体定义显示顺序，而无需重复定义本身。添加新地图图层只需一条注册表条目 — 平面地图和 3D 地球仪均自动从此目录派生其开关面板。
