---
title: "CORS"
description: "World Monitor 中跨域请求保护的工作原理，以及添加新 API 端点时的处理方式 —— 允许列表、预检请求与 Worker 门控。"
---
...
来自任何其他源的请求会收到 403 响应。**没有** `Origin` 头的请求（服务器到服务器、curl）会被放行——`isDisallowedOrigin` 检查仅在源存在且不在允许列表中时才拦截。
