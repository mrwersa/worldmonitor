---
title: "Changelog"
description: "All notable changes to World Monitor organized by release — new features, bug fixes, API additions, and breaking changes across every version."
rss: true
---
...
- **MCP cache-tool default cap**: `get_country_macro`, `get_eu_housing_cycle`, `get_eu_quarterly_gov_debt`, and `get_eu_industrial_production` now cap returned country maps to 30 entries when `limit` is omitted. This aligns these tools with the documented MCP `DEFAULT_LIST_LIMIT` behavior used by other cache tools, but it is a consumer-facing default change: integrations that need the full country set must pass `limit: 0` explicitly.
...
- `POST /api/scenario/v1/run-scenario` **returns `202 Accepted` again** on successful enqueue, restoring the pre-sebuf-migration contract (supersedes the `200 OK` entry below). The response now also carries a `Location` header pointing at the `get-scenario-status` poll URL (same value as the body's `statusUrl`), completing the standard REST async-job pattern. The gateway upgrades the sebuf-generated 200 via a status-override side-channel; error statuses (400/403/429/502) are unchanged. Treat any `2xx` as success — the interim guidance to branch on `response.body.status === "pending"` remains valid.
