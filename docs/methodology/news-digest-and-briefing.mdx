...
- Per-story `whyMatters` uses an analyst endpoint first, then a direct
  OpenRouter-chain fallback, then the baseline stub if every LLM layer fails.
...
uses DeepSeek V4 Flash (`deepseek/deepseek-v4-flash`, reasoning disabled) for
those surfaces when the LLM layer is enabled. Digest prose and
...
tries OpenRouter first with `deepseek/deepseek-v4-flash`, then falls back to
Groq `llama-3.3-70b-versatile`, and sends both providers temperature
