...
| **Browser-first compute where appropriate** | Clustering, local ML, geolocation, surge detection, and offline fallbacks run client-side. Server-authoritative APIs publish CII/CRI scores, briefs, forecasts, MCP tools, and cached operational data with documented methodology and provenance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **Contract-first APIs**             | Every domain intelligence endpoint starts as a `.proto` definition with field validation, HTTP annotations, and examples. Code generation produces typed TypeScript clients and servers, eliminating schema drift. Operational endpoints (auth, billing, MCP, health, bootstrap, notifications, user workflows) remain hand-written under `api/`. Breaking changes to contract APIs are caught automatically at CI time.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Run anywhere**                    | Same codebase produces six specialized variants (geopolitical, tech, finance, commodity, happy, energy) from a single Vercel deployment and deploys to Vercel (web), Railway (relay), Tauri (desktop), and PWA (installable). Desktop sidecar mirrors all cloud API handlers locally. Service worker caches map tiles for offline use while keeping intelligence data always-fresh (NetworkOnly).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **Type-safe data flow**             | Discriminated union markers (`_kind` field), proto-generated typed clients/servers, and exhaustive `switch` matching ensure compile-time safety across 15+ marker types, 34 service domains, and 56 map layers. Adding a new data type produces compiler errors at every unhandled site.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
**Intelligence gap awareness** — professional intelligence assessments always note what they *don't* know. The data freshness tracker explicitly reports "what can't be seen" — 35 sources with status categorization (fresh, stale, very_stale, no_data, error, disabled). Two browser-tracker sources (GDELT, RSS) are flagged as `requiredForRisk`, meaning their absence can force the Strategic Risk panel into its hard insufficient-data state. That flag is narrower than the full CII scoring dependency graph: UCDP events, ACLED conflict, news summaries, cyber threats, and other score inputs are monitored through source-specific health plus the `riskScores` health/cache signal-density coverage. When a critical data source goes down, the system displays the gap prominently rather than silently omitting it, preventing false confidence from incomplete data.
...
**34 service domains** cover every data vertical:
...
| `consumer-prices`| Consumer price overview, basket series, retailer spreads, inflation movers |
| `forecast`       | Scenario forecasts, simulation packages and outcomes |
| `health`         | Disease outbreaks, air quality alerts            |
| `imagery`        | Satellite imagery search                         |
| `leads`          | Contact submission, interest registration        |
| `natural`        | Natural disaster events (USGS, GDACS, EONET)     |
| `radiation`      | Radiation observations (RadNet, Safecast)        |
| `resilience`     | Country resilience scores and rankings           |
| `sanctions`      | Sanctions pressure, entity lookup                |
| `scenario`       | Scenario simulations, templates, run status      |
| `shipping`       | Route risk intelligence, delivery webhooks       |
| `thermal`        | Thermal escalation detections                    |
| `webcam`         | Live webcam directory and imagery                |
...
**Edge gateway** — proto-backed domain entry points (`api/[domain]/v1/[rpc].ts`) register their generated `createServiceRoutes()` descriptors with a flat path router. Each RPC uses the HTTP method declared in its proto/OpenAPI contract: most read endpoints are `GET` (for example, `GET /api/aviation/v1/list-airport-delays`), while mutating and batch operations use `POST`. The shared gateway adds CORS enforcement, a top-level error boundary that hides internal details on 5xx responses, and rate-limit support (`retryAfter` on 429). The same router runs locally via a Vite dev-server plugin (`sebufApiPlugin` in `vite.config.ts`) with HMR invalidation on handler changes.
...
21 Railway cron jobs continuously refresh the Redis cache with pre-computed data from external APIs. Seeds run on configurable schedules from minutes to daily cadence and write both a canonical domain key (for RPC handler lookups) and a bootstrap key (for page-load hydration). This dual-key strategy ensures that bootstrap hydration and RPC handlers always agree on data format and freshness.
...
| `seed-ucdp-events` | UCDP GED API | 6 hours | `conflict:ucdp-events:v1` |
...
| `seed-security-advisories` | 24 advisory RSS/Atom feeds via relay proxy | 1 hour | `intelligence:advisories-bootstrap:v1` |
...
World Monitor uses 60+ Vercel Edge Functions as a lightweight API layer, split into two generations. Legacy endpoints in `api/*.js` each handle a single data source concern — proxying, caching, or transforming external APIs. The newer proto-first endpoints use **per-domain thin entry points** — 34 separate edge functions, each importing only its own handler module. This replaced the original monolithic gateway that loaded all 34 domains on every cold start. Each domain's function tree-shakes to include only its dependencies, reducing cold-start time by ~85% (sub-100ms for most endpoints vs. 500ms+ with the monolithic handler). A shared `server/gateway.ts` provides common routing logic. Both generations coexist, with new features built proto-first. This architecture avoids a monolithic backend while keeping API keys server-side:

- **RSS Proxy** — domain-allowlisted proxy for 500+ feeds, preventing CORS issues and hiding origin servers. The Edge proxy and Railway relay both keep redirects manual and re-check the RSS domain allowlist before every hop. Feeds from domains that block Vercel IPs are automatically routed through the Railway relay.
...
The original monolithic edge gateway (`api/[domain]/v1/[rpc].ts`) imported every service domain handler into a single function. When any RPC was called, the edge runtime loaded the entire handler graph — initializing Redis clients, parsing configuration, and importing utility modules for every domain even though only 1 was needed.

This was split into per-domain thin entry points — one per service domain — each importing only its own handler module. The shared gateway (`server/gateway.ts`) provides common routing logic, but each domain's edge function tree-shakes to include only its dependencies.
...
All six dashboard variants (World Monitor, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor, Energy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
