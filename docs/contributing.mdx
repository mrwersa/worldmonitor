---
title: "Contributing"
description: "Code style, pull-request process, contribution types, and security practices for contributors sending patches to the World Monitor project."
---
...
Public API entry points validate and clamp all parameters:

| Endpoint | Validation |
|----------|------------|
| `GET /api/market/v1/list-market-quotes` | Stock/index `symbols` list; empty returns defaults |
| `GET /api/market/v1/list-crypto-quotes` | CoinGecko `ids` list; empty returns defaults |
| `GET /api/polymarket` | Legacy Polymarket proxy: order field allowlist, limit clamped 1-100 |
