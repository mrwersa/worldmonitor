...
- [x] **Self-host entitlement bypass** — `WM_SELF_HOST=1` (server) + `VITE_SELF_HOST=1` (build arg → client) short-circuits Pro/enterprise gates. Docker Compose defaults the flag to `1` (override with `WM_SELF_HOST=0` for hosted-gate testing); non-Docker hosted builds remain fail-closed. Unlocked panels no longer retain PRO badges. Hosted-only Notifications and MCP management are labeled/hidden honestly rather than presenting unusable upgrade prompts; local persistence for those surfaces remains follow-up work.
...
- [x] **Native Persian RSS feeds** — publisher-owned IRNA, Mehr, and ISNA feeds are registered as distinct `lang: 'fa'` sources in both client and server inventories, preserving the existing English IRNA/Mehr feeds. They are default-enabled but locale-gated, carry state-affiliation/risk metadata and source tiers, and appear in the public feed inventory.
