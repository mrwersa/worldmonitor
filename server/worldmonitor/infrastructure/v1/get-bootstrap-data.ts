...
// this RPC bootstrap surface must also stop shipping iranEvents while
// disabled, mirroring api/bootstrap.js. It reads the SHARED
// BOOTSTRAP_CACHE_KEYS, so the gate lives here. Set IRAN_EVENTS_ENABLED=true
...
function buildRegistry(req: GetBootstrapDataRequest): Record<string, string> {
  ...
  if (req.tier === 'slow' || req.tier === 'fast') {
    ...
    // Copy so the conditional delete below never mutates the shared registry.
